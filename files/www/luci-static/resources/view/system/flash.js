...
return view.extend({
	load: function() {
		var tasks = [
			...
			L.resolveDefault(fs.stat('/dev/mmcblk0p1'), {})
		];
	...
	handleBurn: function(ev) {
		if (!confirm(_('Really write OpenWrt to flash?')))
			return;

		fs.exec('/usr/bin/burn.sh')
			.then(L.bind(function(btn, res) {
				if (res.code != 0) {
					ui.addNotification(null, [
						E('p', _('The install to Flash failed:')),
						res.stdout ? E('pre', {}, [ res.stdout ]) : ''
					]);
				} else {
					ui.addNotification(null, E('p', _('The install to Flash was successful. You can now power off the device,'+
										' remove the SD-Card and boot without it.')), 'info');
				} 
			}, this, ev.target))
	},
	...
	render: function(rpc_replies) {
		var has_sysupgrade = (rpc_replies[0].type == 'file'),
		    ...
		    sdcard_avail = (rpc_replies[5].type == 'block'),
		...
		if(sdcard_avail) {
			o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('Install OpenWrt to Flash'), _('Click "Flash" to install OpenWrt to the internal flash of this Baltos device. Changes to the Configuration on the SD-Card are not transfered. "Backup" and "Restore" could be used for that. All data in the flash will be overwritten.'));
			ss = o.subsection;

			o = ss.option(form.Button, 'burn', _('Install to Flash'));
			o.inputstyle = 'action important';
			o.inputtitle = _('Flash');
			o.onclick = this.handleBurn;
		}
